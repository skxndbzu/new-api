...
func TestAdvancedCustomMatchPathForModelAcceptsPlaygroundAliases(t *testing.T) {
	config := &AdvancedCustomConfig{
		Routes: []AdvancedCustomRoute{
			{IncomingPath: "/v1/chat/completions", UpstreamPath: "/chat"},
			{IncomingPath: "/v1/images/generations", UpstreamPath: "/generate"},
			{IncomingPath: "/v1/images/edits", UpstreamPath: "/edit"},
		},
	}
	require.NoError(t, config.Validate())
	...
	tests := []struct {
		...
		{requestPath: "/pg/chat/completions", wantUpstreamPath: "/chat"},
		{requestPath: "/pg/images/generations", wantUpstreamPath: "/generate"},
		{requestPath: "/pg/images/edits", wantUpstreamPath: "/edit"},
	}

	for _, tt := range tests {
		t.Run(tt.requestPath, func(t *testing.T) {
			route, ok := config.MatchPathForModel(tt.requestPath, "gpt-image-1")
			require.True(t, ok)
			assert.Equal(t, tt.wantUpstreamPath, route.UpstreamPath)
		})
	}
}
